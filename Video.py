...
# stream = cv2.VideoCapture("D:/videos/GOPRO/GH010028.MP4")
# fps = round(stream.get(cv2.CAP_PROP_FPS))
# w = round(stream.get(cv2.CAP_PROP_FRAME_WIDTH))
# h = round(stream.get(cv2.CAP_PROP_FRAME_HEIGHT))

# fourcc = cv2.VideoWriter_fourcc('M', 'J', 'P', 'G')
# video_writer = cv2.VideoWriter('D:/videos/GOPRO/GH010028_converted.avi', fourcc, fps, (w, h))

ground_colors = get_colors(len(class_names))

reader = imageio.get_reader("D:/videos/GOPRO/GH010025.MP4")
fps = reader.get_meta_data()['fps']

writer = imageio.get_writer('D:/videos/GOPRO/GH010025_converted.avi', fps=fps)
...
for frame in reader:
    ...
    writer.append_data(masked_image)
...
print("Total processing time: {:.2f}s".format(end_reading - start_reading))
